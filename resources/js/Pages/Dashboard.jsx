...
export default function Dashboard({ auth, videos }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
                            ...
                            <li key={video.title} className="relative">
                                <div
                                    ...
                                    <button type="button" className="absolute inset-0 focus:outline-none">
                                        <span className="sr-only">View details for {video.created_at}</span>
                                    </button>
                                </div>
                                <p className="mt-2 block text-sm font-medium text-gray-900 truncate pointer-events-none">{video.title}</p>
                                <p className="block text-sm font-medium text-gray-500 pointer-events-none">{video.created_at}</p>
                            </li>
                        ))}
                    </ul>
                </div>
